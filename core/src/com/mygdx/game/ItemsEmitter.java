...
public class ItemsEmitter {//Класс генерации объектов класса Item в игру
    private Item[] items;
    ...
    public void render(SpriteBatch batch) {
        for (int i = 0; i < items.length; i++) {
            if (items[i].isActive()) {
                int frameIndex = (int)(items[i].getTime() / 0.15f) % regions[items[i].getType().index].length;
    ...
    public void generateRandomItem(float x, float y, int count, float probability) {
        //Рандомно выбирает какой из типов усеилений будет
        //выдан в игру и создаёт его
